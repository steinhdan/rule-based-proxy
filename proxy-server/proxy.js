/**
 * Slight modification of https://github.com/TooTallNate/proxy
 * Modifications:
 *  - Added a provisioning callback in onrequest and onconnect
 *  - Added helper to respond with a "HTTP 402 - Payment required" response
 */
...
var version  = '1.0'; // require('./package.json').version;
...
var allowRequest = undefined;

function setup (server, options, allowRequestFn) {
    allowRequest = allowRequestFn;
...
function onrequest (req, res) {
    ...
    if (!allowRequest(req, '' + req.url)) {
        return requestPayment(req, res);
    }
...
function onconnect (req, socket, head) {
    ...
    if (!allowRequest(req, ('https://' + req.url + '/[ENCRYPTED_PATH]').replace(/:443\//, '/'))) {
        return requestPayment(req, res);
    }
...
/**
 * Sends a "402 Payment Required" HTTP response to the `socket`.
 *
 * @api private
 */
...
function requestPayment (req, res) {
    // request Basic proxy authorization
    debug.response('requesting payment for "%s %s"', req.method, req.url);
